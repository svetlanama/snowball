...
df=pd.DataFrame(c_values)
df.columns=['term', 'cvalue']
df['term']=df['term'].map(lambda x: str(x).strip())
df.sort_values(by=['cvalue'], inplace=True, ascending=False)
df=df[ df['cvalue']>0 ]
total=sum(df['cvalue'])/2
print('total',total)
df['ccv']=df['cvalue'].cumsum()
df=df[ df['ccv']<=total ]
df.set_index('term')[['cvalue']].to_csv(out_terms)
